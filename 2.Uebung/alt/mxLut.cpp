...
{   
    ...
    if (nlhs > 1 || nrhs < 2|| nrhs > 2)
        mxErrMsgTxt("Usage: picOut = mxLut(picIn, lut);");
    
    // check input: Image
    if (!mxIsUint8(prhs[0]))
        mxErrMsgTxt("mxLut can only process uint8 Image");
    
    // check input: lut - uint8, complex, [1] [256]
    mrows = mxGetM(prhs[1]);
    ncols = mxGetN(prhs[1]);
    if (!mxIsUint8(prhs[1]))
        mxErrMsgTxt("mxLut can only process uint8 Look-up table");
    else if (!(mrows == 1 && ncols == 256))
        mxErrMsgTxt("Look-up table size must be [1][256]");
